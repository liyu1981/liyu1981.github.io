...
5. 内核是不直接过问具体事情的，它会启动一个新的应用程序，来负责启动其他的应用程序。
...
实际上，在SMF的支撑下，DogeOS以及SmartOS的服务体系发展的异常的 。。。强大。支持系统的，不再是一些独立的互不相干的服务，而是一张服务的网。
...
DogeOS，以及类illumos的OS，其服务体系采用了SMF机制，是一个具有依赖关系的服务网络。理解这些后台服务，需要从它们之间的依赖关系开始着手。

(BTW: Linux的各个发行版，目前也开始模仿SMF机制引入了不同的init替代品，这其中最有名的，即是RedHat主导的[systemd](http://en.wikipedia.org/wiki/Systemd)，以及Ubuntu中所采用的[upstrart](http://en.wikipedia.org/wiki/Upstart)。)